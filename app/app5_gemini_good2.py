...
# --- THEME COLORS ---
PRIMARY_COLOR = "#e40173"  # New color requested
BG_LIGHT_COLOR = "#fcebf4" # Lighter version for backgrounds
...
<style>
    ...
    /* FIX: Explicit Sidebar Styling for RTL */
    section[data-testid="stSidebar"] {{
        direction: {txt['dir']};
        text-align: {txt['align']};
    }}
    
    /* FIX: Adjust Sidebar Widgets alignment */
    ...
    .stButton > button:hover {{
        background-color: #c20061; /* Darker shade on hover */
        color: white;
    }}
    
    /* Malignant Banner - Updated Color */
    .malignant-banner {{
        background-color: {BG_LIGHT_COLOR};
        border-{'right' if lang == 'fa' else 'left'}: 5px solid {PRIMARY_COLOR};
        padding: 20px;
        border-radius: 8px;
        margin-bottom: 20px;
    }}
...
if menu == txt['nav_dash']:
    ...
    if uploaded_file and st.session_state['analyzed']:
        ...
        with col_pred:
            # Prediction Banner (Colors Updated)
            st.markdown(f"""
            <div class="malignant-banner">
                <div style="color: #666; font-size: 0.9rem;">{txt['pred_title']}</div>
                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 10px;">
                    <div>
                        <div style="color: {PRIMARY_COLOR}; font-size: 2.2rem; font-weight: 800;">{txt['class_mal']}</div>
                        <div style="color: #444;">{txt['desc_mal']}</div>
                    </div>
                    <div style="text-align: center;">
                         <div style="color: {PRIMARY_COLOR}; font-size: 1.8rem; font-weight: 700;">98.2%</div>
        ...
        with col_proto:
            ...
            <div class="protocol-card">
                ...
                <div style="text-align: {txt['align']};">
        ...
        with st.container():
            ...
            with xc2:
                ...
                # Placeholder: In a real app, you would overlay the heatmap
...
# --- PAGE: ABOUT US ---
    ...
    st.markdown(f"""
    <div style="background-color: #f8f9fa; padding: 25px; border-radius: 10px; border-{'right' if lang=='fa' else 'left'}: 5px solid {PRIMARY_COLOR};">
